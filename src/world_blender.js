...
export function createGameWorld(assets) {
  ...
  function addBlocker(world, x1, x2, z1, z2) {
    blockers[world].push({ x1, x2, z1, z2 });
  }
  ...
  // ---------------------------------------------------------------------------
  place(assets.environment, bunker, [0,0,0]);
  ...
  for (const [x,z] of fixturePositions) {
    place(assets.ceilingLight, bunker, [x,3.76,z]);
  ...
  addBlocker('bunker',4.2,6.45,-6.1,-3.9);

  place(assets.electrical, bunker, [-4.85,.10,6.88], [0,0,0], .90);
  addBlocker('bunker',-6.3,-3.3,6.3,6.75);

  place(assets.lockers, bunker, [-2.55,0,6.55], [0,0,0], .84);
  addBlocker('bunker',-4.0,-1.1,6.0,6.78);

  place(assets.bench, bunker, [2.35,0,6.30], [0,Math.PI,0], .88);
  addBlocker('bunker',.7,4.0,5.55,6.78);

  place(assets.clutter, bunker, [4.45,0,2.80], [0,-Math.PI/2,0], .92);
  place(assets.statusBoard, bunker, [0,1.15,7.17], [0,Math.PI,0], .90);
  ...
  bunkerCameraPlacements.forEach(([p,r])=>place(assets.wallCamera,bunker,p,r,.72));
  ...
  addBlocker('bunker',3.55,6.30,4.0,6.0);
  ...
  // Blast door + fully Blender-built access control panel
  const blast = place(assets.blastDoor, bunker, [0,0,-7.30]);
  ...
  const exitPanel = place(assets.accessControl, bunker, [-1.82,.60,-6.96], [0,0,0], .62);
  ...
  // ---------------------------------------------------------------------------
  place(assets.exteriorGround, outside, [0,0,0]);
  place(assets.exteriorEntrance, outside, [0,0,-17]);
  ...
  floodPositions.forEach(([x,y,z])=>{
    place(assets.floodlight,outside,[x,y,z]);
  ...
  [[-11,0,-14,0],[11,0,-14,.7],[-15,0,5,.2],[13,0,12,-.5],[4,0,2,.4]].forEach(([x,y,z,r])=>place(assets.rubble,outside,[x,y,z],[0,r,0],1));

  // Surface access uses the same Blender keypad asset.
  const returnPanel = place(assets.accessControl,outside,[-2.15,.55,-13.55],[0,0,0],.64);
